...
function mapWeatherApiCodeToWmoLike(code: number): number {
  ...
  if (
    [
      1063, 1150, 1153, 1180, 1183, 1186, 1189, 1192, 1195, 1240, 1243, 1246, 1204, 1207, 1249, 1252, 1072, 1168, 1171,
    ].includes(code)
  )
    return 63 // rain / drizzle
  if ([1066, 1069, 1114, 1117, 1210, 1213, 1216, 1219, 1222, 1225, 1255, 1258, 1237, 1261, 1264].includes(code))
    return 75 // snow / ice
...
export async function GET(req: Request) {
  try {
    ...
    console.log('Fetching weather data from:', url.toString().replace(key, '[REDACTED]'))
    ...
    if (!data?.current) {
      ...
      return NextResponse.json({ error: "Invalid response from WeatherAPI" }, { status: 500 })
    }
    ...
    const normalized = {
      current: {
        ...
        apparent_temperature: data.current?.feelslike_c ?? data.current?.temp_c ?? 0,
        relative_humidity_2m: data.current?.humidity ?? 0,
        wind_speed_10m: data.current?.wind_kph ? Number((data.current.wind_kph / 3.6).toFixed(1)) : 0, // kph -> m/s
        is_day: data.current?.is_day === 1 ? 1 : 0,
        weather_code: mapWeatherApiCodeToWmoLike(data.current?.condition?.code ?? 1003),
      },
      daily: (() => {
        ...
        return {
          ...
          weather_code: daysArr.map((d: any) => mapWeatherApiCodeToWmoLike(d?.day?.condition?.code ?? 1003)),
